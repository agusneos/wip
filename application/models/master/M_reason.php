...
{    
    ...
    function index()
    {
        ...
        $offset = ($page-1)*$rows;      
        $sort   = isset($_POST['sort']) ? strval($_POST['sort']) : 'm_reason_id';
    ...
    function create($m_reason_txt)
    {
        $query = $this->db->insert(self::$table1,array(
            'm_reason_txt'    => $m_reason_txt
        ));
        if($query){
    ...
    function update($m_reason_id, $m_reason_txt) {
        $this->db->where('m_reason_id', $m_reason_id);
    ...
    function delete($m_reason_id)
    {
        $query = $this->db->delete(self::$table1, array('m_reason_id' => $m_reason_id));
        if($query){
            return json_encode(array('success'=>true));
        }
        else{
...
/* Location: ./application/models/master/m_proccat.php */